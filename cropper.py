...
HAARCASCADE_PROFILEFACE = './utils/haarcascade_profileface.xml'
HAARCASCADE_FRONTALFACE = './utils/haarcascade_frontalface_default.xml'
...
'''
def center_from_mtcnn(image, face_number=1, face_threshold=0.95, area_threshold=0.002):
    
    height, width, channel = image.shape
        
    boxes, scores = MTCNN_MODEL.detect(Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB)))
...
Corner detection.
'''
def center_from_good_features(image):
...
'''
def center_from_saliency(image, save_saliency=False, save_dir='./'):
...
def detect(image):
    ...
    face_mtcnn = center_from_mtcnn(image)
    
    if face_mtcnn:
        cx, cy, bbox = face_mtcnn
    else:
        face_center = center_from_faces(gray)

        center = {'x': 0, 'y': 0}
        if not face_center:
            center_features = center_from_good_features(gray)
            center_saliency = center_from_saliency(image)
            center['x'] = (center_features['x']+center_saliency[0]) // 2
            center['y'] = (center_features['y']+center_saliency[1]) // 2
        else:
            features_center = center_from_good_features(gray)
...
'''
def crop(image_dir, completeness=False, target_size=(500,500)):
    ...
    maximum_x, maximum_y, flag = detect(image)
